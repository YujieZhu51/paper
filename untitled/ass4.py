...
sess=tf.Session()
saver = tf.train.import_meta_graph('.//ass4.meta')
saver.restore(sess, tf.train.latest_checkpoint('./')) #加载图和参数变量
...
w_full1 = graph.get_tensor_by_name("w_full1:0")
w_full2 = graph.get_tensor_by_name("w_full2:0")
b_full1 = graph.get_tensor_by_name("b_full1:0")
b_full2 = graph.get_tensor_by_name("b_full2:0")
...
acc2 = sess.run(accuracy, feed_dict={x: testImage.eval(), y: testLabel1.eval()}) # test要全部

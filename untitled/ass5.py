...
y_num = tf.placeholder(tf.float32, [None, 10])
y_name = tf.placeholder(tf.float32, [None, 5])
...
w_full2_num = tf.Variable(tf.truncated_normal([256, 10], stddev=0.1))
b_full2_num = tf.Variable(tf.constant(0.1, shape=[10]))
w_full2_name = tf.Variable(tf.truncated_normal([256, 5], stddev=0.1))
b_full2_name = tf.Variable(tf.constant(0.1, shape=[5]))

y_pred_num = tf.nn.softmax(tf.matmul(h_fc1_drop, w_full2_num) + b_full2_num)
y_pred_name = tf.nn.softmax(tf.matmul(h_fc1_drop, w_full2_name) + b_full2_name)

loss_num = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y_num, logits=y_pred_num))
loss_name = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y_name, logits=y_pred_name))
weight = 0.7
loss = weight*loss_num + (1-weight)*loss_name

train = tf.train.AdamOptimizer(learning_rate=0.001).minimize(loss)
correct_num = tf.equal(tf.argmax(y_num, 1), tf.argmax(y_pred_num, 1))
correct_name = tf.equal(tf.argmax(y_name, 1), tf.argmax(y_pred_name, 1))
accuracy_num = tf.reduce_mean(tf.cast(correct_num, tf.float32))
accuracy_name = tf.reduce_mean(tf.cast(correct_name, tf.float32))
...
with tf.Session() as sess:
    ...
    for epoch in range(7):
        ...
        for batch in range(n_batch):
            count = count + 1
            print(count)
            batch_x, batch_y_num, batch_y_name = next_batch(batch_size)
            sess.run(train, feed_dict={x: batch_x, y_num: batch_y_num, y_name: batch_y_name})
        acc_num = sess.run(accuracy_num, feed_dict={x: batch_x, y_num: batch_y_num, y_name: batch_y_name})
        acc_name = sess.run(accuracy_name, feed_dict={x: batch_x, y_num: batch_y_num, y_name: batch_y_name})
        print("迭代" + str(epoch + 1) + " acc number: " + str(acc_num) + " acc name: " + str(acc_name))
    acc2_num = sess.run(accuracy_num, feed_dict={x: testImage.eval(), y_num: testLabel1.eval(), y_name: testLabel2.eval()})
    acc2_name = sess.run(accuracy_name, feed_dict={x: testImage.eval(), y_num: testLabel1.eval(), y_name: testLabel2.eval()})
    print("test" + " acc number: " + str(acc2_num) + " acc name: " + str(acc2_name))
